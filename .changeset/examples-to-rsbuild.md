...
"@lynx-example/css-global": patch
"@lynx-example/css-modules": patch
"@lynx-example/css-postcss": patch
...
"@lynx-example/text-composition": patch
...
Build with Rsbuild and `pluginLynx` instead of Rspeedy. The entry keeps its `main` name, so the artifact names are unchanged.
